 ...
 switch (a.measure.data_type)
    ...
    case CF_DATA_TYPE_REAL:
        ...
        if (a.measure.history_type && (strcmp(a.measure.history_type, "weekly") == 0))
           {
           if ((slot = MonRegisterSlot(handle, pp->comment ? pp->comment : "User defined measure",
                                        a.measure.units ? a.measure.units : "unknown", 0.0f, 100.0f, true)) < 0)
              {
              return result;
              }
           
           result = PromiseResultUpdate(result, MonExtractValueFromStream(ctx, handle, stream, a, pp, &this[slot]));
           Log(LOG_LEVEL_VERBOSE, "Setting Mon slot %d=%s to %lf", slot, handle, this[slot]);
           }
        else if (a.measure.history_type && (strcmp(a.measure.history_type, "log") == 0))
           {
           Log(LOG_LEVEL_VERBOSE, "Promise to log a numerical value");
           MonLogSymbolicValue(ctx, handle, stream, a, pp, &result);
           }
        else                    /* static */
           {
           Log(LOG_LEVEL_VERBOSE, "Promise to store a static numerical value");
           result = PromiseResultUpdate(result, MonExtractValueFromStream(ctx, handle, stream, a, pp, &new_value));
           MonNamedEvent(handle, new_value);
           }
        break;
