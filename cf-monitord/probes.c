...
void MonOtherInit(void)
{
 ...
 Log(LOG_LEVEL_VERBOSE, "Starting initialization of static monitoring probes.");
 ...
 for (i = 0; i < sizeof(ENTERPRISE_PROBES) / sizeof(ENTERPRISE_PROBES[0]); ++i)
    ...
    Log(LOG_LEVEL_VERBOSE, "Initialization of static monitoring probes is finished.");
}
...
void MonOtherGatherData(double *cf_this)
{
    ...
    Log(LOG_LEVEL_VERBOSE, "Gathering data from static monitoring probes.");
    ...
    Log(LOG_LEVEL_VERBOSE, "Gathering data from static monitoring probes is finished.");
}
