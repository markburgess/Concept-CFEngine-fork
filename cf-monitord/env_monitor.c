...
/* Discard any apparently anomalous behaviour before renormalizing database */
 ...
 for (i = 0; i < CF_OBSERVABLES; i++)
    ...
    This = RejectAnomaly(CF_THIS[i], lastweek_vals->Q[i].expect, lastweek_vals->Q[i].var, LOCALAV.Q[i].expect, LOCALAV.Q[i].var);
    ...
    if (This > 0)
       {
       Log(LOG_LEVEL_VERBOSE, "[%d] %s Sample=%lf, Average=%lf -> Deviation=%lf - smoothed [%lf#%lf]", i, name, newvals.Q[i].q, newvals.Q[i].expect, sqrt(newvals.Q[i].var), LOCALAV.Q[i].expect, sqrt(LOCALAV.Q[i].var));
       }
    }
...
*/
 ...
 for (i = 0; i < CF_OBSERVABLES; i++)
    ...
    position = CF_GRAINS * (0.5 + (int) (CF_THIS[i] - av->Q[i].expect)/(1.0 + MAX.Q[i].q));

    Log(LOG_LEVEL_VERBOSE," -> Positioning [%d] %lf at class position %d/%d\n", i, CF_THIS[i], position,CF_GRAINS);
